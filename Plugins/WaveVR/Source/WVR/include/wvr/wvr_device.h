...
typedef enum {
    ...
    WVR_RecenterType_YawAndPosition = 2, /**< Adjust the Yaw angle, and also reset user's position XYZ to Virtual World's center */
    ...
    WVR_RecenterType_Position = 4, /**< Reset user's position XYZ to Virtual World's center */
} WVR_RecenterType;
 ...
 * @return int32_t, bitmask for WVR_InputId. A value of -1 means an error has occured.
 * @version API Level 1
*/
...
/**
 * @brief Function to get the count for WVR_InputType. This is a calculated WVR_InputType count
 * based on the WVR_SetInputRequest paired result.
 *
...
/**
 * @brief Function to trigger vibration with float amplitude.
 *
 * @param type Indicates what device type. (refer to @ref WVR_DeviceType)
 * @param id Indicates the @ref WVR_InputId to trigger vibration.
 * @param durationMicroSec The vibration duration.
 * @param frequency Number of vibration based on durationMicroSec.
 * @param amplitude float intensity of the vibration. The default value is 0.0f. The valid interval: 0.0f~1.0f.
 * @version API Level 7
 * @note Effective with Runtime version 7 or higher
*/
extern WVR_EXPORT void WVR_TriggerVibrationScale(WVR_DeviceType type, WVR_InputId inputId = WVR_InputId_Max, uint32_t durationMicroSec = 65535, uint32_t frequency = 1, float amplitude = 0.0f);
 ...
 * as the origin model will be relative to this new zero pose. Take effects until System Recentered or WVR_RecenterType_Disabled.
 ...
 *
 * @param dev_type Device type to query the error state. (refer to @ref WVR_DeviceType)
 * @param error_type It is the bitmask that records all the of current states of device errors which has occurred on the specified device. (refer to @ref WVR_DeviceErrorStatus)
 * @return bool, true when specific error occurred, false no error.
 * @version API Level 3
 * @note Effective with Runtime version 3 or higher
 */
 ...
 * @version API Level 6
 */
extern WVR_EXPORT bool WVR_SetControllerPoseMode(WVR_DeviceType type, WVR_ControllerPoseMode mode);

/**
 * @brief Function to get current enabled controller pose mode.
 * @param type Indicates what device type. (refer to @ref WVR_DeviceType). This should be the controller type.
 * @param mode Returns the current enabled controller pose mode. (It should not be a null pointer.)
 * @return True means mode obtained successfully; False means mode obtained unsuccessfully.
 * @version API Level 6
 */
extern WVR_EXPORT bool WVR_GetControllerPoseMode(WVR_DeviceType type, WVR_ControllerPoseMode *mode);

/**
 * @brief Function to get the offset data of current enabled controller pose mode.
 * @param type Indicates what device type. (refer to @ref WVR_DeviceType). This should be the controller type.
 * @param mode Indicates what controller pose mode (refer to @ref WVR_ControllerPoseMode).
 * @param translation Returns the translation offset of assigned mode. (It should not be a null pointer.)
 * @param quaternion Returns the quaternion offset of assigned mode. (It should not be a null pointer.)
 * @param unrotatedTranslation True means the returned parameter translation is raw; False means the returned parameter translation is rotated by the returned parameter quaternion.
 ...
 * @version API Level 6
 */
extern WVR_EXPORT bool WVR_GetControllerPoseModeOffset(WVR_DeviceType type, WVR_ControllerPoseMode mode, WVR_Vector3f_t *translation, WVR_Quatf_t *quaternion, bool unrotatedTranslation = false);
 ...
 * @version API Level 8
 */
extern WVR_EXPORT bool WVR_IsDeviceTableStatic(WVR_DeviceType type);
