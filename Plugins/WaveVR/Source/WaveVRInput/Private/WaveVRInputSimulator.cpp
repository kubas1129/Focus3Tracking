...
bool WaveVRInputSimulator::IsButtonPressed(EWVR_DeviceType device, EWVR_InputId button_id)
{
	...
	case EWVR_DeviceType::DeviceType_Controller_Left:
		if (button_id == EWVR_InputId::Touchpad)
			_result = PlayerController->IsInputKeyDown(EKeys::LeftMouseButton);
		if (button_id == EWVR_InputId::X)
			_result = PlayerController->IsInputKeyDown(EKeys::X);
		if (button_id == EWVR_InputId::Y)
			_result = PlayerController->IsInputKeyDown(EKeys::Y);
		break;
	case EWVR_DeviceType::DeviceType_Controller_Right:
		if (button_id == EWVR_InputId::Touchpad)
			_result = PlayerController->IsInputKeyDown(EKeys::RightMouseButton);
		if (button_id == EWVR_InputId::A)
			_result = PlayerController->IsInputKeyDown(EKeys::A);
		if (button_id == EWVR_InputId::B)
			_result = PlayerController->IsInputKeyDown(EKeys::B);
		break;
	default:
		break;
	}
