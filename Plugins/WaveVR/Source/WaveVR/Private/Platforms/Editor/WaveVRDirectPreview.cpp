...
DP_InitError WaveVRDirectPreview::ReadSettingsAndInit() {
	...
	LOGW(WVRDirectPreview, "readSettingsAndInit : ConntectType = %d , WifiIP = %s , EnablePreviewImage = %s , RegularlySaveImages = %s , FPS = %d",
		 ConnectType, PLATFORM_CHAR(*DeviceWiFiIP), EnablePreviewImage ? PLATFORM_CHAR(*FString("true")) : PLATFORM_CHAR(*FString("false")), RegularlySaveImages ? PLATFORM_CHAR(*FString("true")) : PLATFORM_CHAR(*FString("false")), mFPS);
