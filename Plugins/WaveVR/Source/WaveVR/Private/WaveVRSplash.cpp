...
void FWaveVRSplash::Show()
{
	...
	if(bSplashScreenRendered) {
		...
		const WVR_OverlayPosition position = {0.0f, 0.0f, -0.55f};
	...
	if (!bSplashScreenRendered)
	{
		...
		RHICmdList.BeginRenderPass(RPInfo, TEXT("BlitSplashTexture"));
		{
			...
			if (SplashTexture != nullptr && SplashTexture->IsValidLowLevel())
			{
				...
				PixelShader->SetParameters(RHICmdList, TStaticSamplerState<SF_Trilinear>::GetRHI(), SplashTexture->GetResource()->TextureRHI);
...
bool FWaveVRSplash::AllocateSplashWrapper_RenderThread() {
	...
	uint32 InSizeX = 2048; //mRender->GetSingleEyePixelWidth();
	uint32 InSizeY = 2048; //mRender->GetSingleEyePixelHeight();
	EPixelFormat InFormat = EPixelFormat::PF_A2B10G10R10;
	uint32 InNumMips = 1;
	uint32 InNumSamples = 1;
	ETextureCreateFlags InFlags = ETextureCreateFlags::RenderTargetable;
	FRHIResourceCreateInfo CreateInfo(TEXT("AllocateSplashWrapper_RenderThread"));
	SplashWrapperRHIRef = OpenGLDynamicRHI->RHICreateTexture2D(InSizeX, InSizeY, (uint8)InFormat, InNumMips, InNumSamples, InFlags, ERHIAccess::CopySrc,CreateInfo);
