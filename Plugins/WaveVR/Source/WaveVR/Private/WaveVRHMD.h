...
public:
	...
	virtual void DrawHiddenAreaMesh(class FRHICommandList& RHICmdList, int32 ViewIndex) const override { LOG_FUNC(); }
	virtual void DrawVisibleAreaMesh(class FRHICommandList& RHICmdList, int32 ViewIndex) const override { LOG_FUNC(); }
	...
	virtual EHMDWornState::Type GetHMDWornState() override { return EHMDWornState::Unknown; }
        ...
        virtual FTexture2DRHIRef GetOverlayLayerTarget_RenderThread(int32 ViewIndex, FIntRect& InOutViewport) override {LOG_FUNC(); return nullptr; }
        virtual FTexture2DRHIRef GetSceneLayerTarget_RenderThread(int32 ViewIndex, FIntRect& InOutViewport) override {LOG_FUNC(); return nullptr; }

        //** FHeadMountedDisplayBase interface not used*/
        virtual FVector2D GetEyeCenterPoint_RenderThread(const int32 ViewIndex) const override { LOG_FUNC(); return FHeadMountedDisplayBase::GetEyeCenterPoint_RenderThread(ViewIndex);}
...
public:
	...
	virtual EStereoscopicPass GetViewPassForIndex(bool bStereoRequested, int32 ViewIndex) const override { LOG_FUNC(); return IStereoRendering::GetViewPassForIndex(bStereoRequested, ViewIndex);}
	virtual uint32 GetLODViewIndex() const override { LOG_FUNC(); return IStereoRendering::GetLODViewIndex(); }
	...
	virtual void StartFinalPostprocessSettings(struct FPostProcessSettings* StartPostProcessingSettings, const enum EStereoscopicPass StereoPassType, const int32 StereoViewIndex) override {
		LOG_FUNC();
		return IStereoRendering::StartFinalPostprocessSettings(StartPostProcessingSettings, StereoPassType, StereoViewIndex);
	}
	virtual bool OverrideFinalPostprocessSettings(struct FPostProcessSettings* OverridePostProcessingSettings,  const enum EStereoscopicPass StereoPassType, const int32 StereoViewIndex, float& BlendWeight) override {
		LOG_FUNC();
		return IStereoRendering::OverrideFinalPostprocessSettings(OverridePostProcessingSettings, StereoPassType, StereoViewIndex, BlendWeight);
	}
	virtual void EndFinalPostprocessSettings(struct FPostProcessSettings* FinalPostProcessingSettings, const enum EStereoscopicPass StereoPassType, const int32 StereoViewIndex) override {
		LOG_FUNC();
		return IStereoRendering::EndFinalPostprocessSettings(FinalPostProcessingSettings, StereoPassType, StereoViewIndex);
	}
...
public:
	...
	virtual bool IsActiveThisFrame_Internal(const FSceneViewExtensionContext& Context) const override { LOG_FUNC(); return FHMDSceneViewExtension::IsActiveThisFrame_Internal(Context); }
...
public:
	...
	void SetFoveationParams(const int32 ViewIndex, WVR_RenderFoveationParams_t& FoveatParams);
	void GetFoveationParams(const int32 ViewIndex, WVR_RenderFoveationParams_t& FoveatParams);
...
private:
	...
	void GenerateDistortionCorrectionVertexBuffer(const int32 ViewIndex);
