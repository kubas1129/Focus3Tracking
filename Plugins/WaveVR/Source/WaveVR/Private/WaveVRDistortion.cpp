  ...
  uint32 VertexDataStride )
{
	...
	FRHIResourceCreateInfo CreateInfo(TEXT("WaveVRDrawIndexedPrimitive"));
...
static void ComputeDitortion(const int32 ViewIndex, const FVector2D input, FVector2D * output)
{
...
void FWaveVRHMD::GenerateDistortionCorrectionVertexBuffer(const int32 ViewIndex)
{
	LOG_FUNC();
	FDistortionVertex** UsingPtr = (ViewIndex == EStereoscopicEye::eSSE_LEFT_EYE) ? &DistortionMeshVerticesLeftEye : &DistortionMeshVerticesRightEye;
	...
	for (uint32 y = 0; y < DistortionPointsY; ++y)
	{
		for (uint32 x = 0; x < DistortionPointsX; ++x)
		{
			...
			{
				ComputeDitortion(ViewIndex, UnDistortedCoord, DistortedCoords);
				//LOGI(WVRHMD, "Approx. %d:  input(%f, %f) G(%f,%f)", i, UnDistortedCoord.X, UnDistortedCoord.Y, DistortedCoords[1].X, DistortedCoords[1].Y);
			...
			const FVector2f ScreenPos = FVector2f(UnDistortedCoord.X * 2.0f - 1.0f, (UnDistortedCoord.Y * 2.0f - 1.0f) * ScreenYDirection);

			const FVector2f OrigRedUV = FVector2f(DistortedCoords[0].X, DistortedCoords[0].Y);
			const FVector2f OrigGreenUV = FVector2f(DistortedCoords[1].X, DistortedCoords[1].Y);
			const FVector2f OrigBlueUV = FVector2f(DistortedCoords[2].X, DistortedCoords[2].Y);

			// Final distorted UVs
			FVector2f FinalRedUV = OrigRedUV;
			FVector2f FinalGreenUV = OrigGreenUV;
			FVector2f FinalBlueUV = OrigBlueUV;
...
void FWaveVRHMD::SetNumOfDistortionPoints(int32 XPoints, int32 YPoints)
{
	...
	GenerateDistortionCorrectionVertexBuffer(EStereoscopicEye::eSSE_LEFT_EYE);
	GenerateDistortionCorrectionVertexBuffer(EStereoscopicEye::eSSE_RIGHT_EYE);
}

void FWaveVRHMD::GetEyeRenderParams_RenderThread(const struct FHeadMountedDisplayPassContext& Context, FVector2D& EyeToSrcUVScaleValue, FVector2D& EyeToSrcUVOffsetValue) const
{
	LOG_FUNC();
...
void FWaveVRHMD::DrawDistortionMesh_RenderThread(struct FHeadMountedDisplayPassContext& Context, const FIntPoint& TextureSize)
{
	LOG_FUNC();
	...
	//LOGI(WVRHMD, "StereoPass: %d ViewportSize (%d, %d) TextureSize (%d, %d)"), (int)View.StereoPass, ViewportSize.X, ViewportSize.Y, TextureSize.X, TextureSize.Y);

	if (View.StereoViewIndex == EStereoscopicEye::eSSE_LEFT_EYE) {
	...
	} else if (View.StereoViewIndex == EStereoscopicEye::eSSE_RIGHT_EYE) {
