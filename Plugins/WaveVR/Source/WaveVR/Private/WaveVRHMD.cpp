...
bool FWaveVRHMD::GetRelativeEyePose(int32 DeviceId, int32 ViewIndex, FQuat& OutOrientation, FVector& OutPosition)
{
	LOG_FUNC();
	OutOrientation = FQuat::Identity;
	OutPosition = FVector::ZeroVector;
	...
	if (DeviceId == IXRTrackingSystem::HMDDeviceId && (ViewIndex == EStereoscopicEye::eSSE_LEFT_EYE || ViewIndex == EStereoscopicEye::eSSE_RIGHT_EYE)) {
		WVR_Eye hmdEye = (ViewIndex == EStereoscopicEye::eSSE_LEFT_EYE) ? WVR_Eye_Left : WVR_Eye_Right;
...
void FWaveVRHMD::AdjustViewRect(const int32 ViewIndex, int32& X, int32& Y, uint32& SizeX, uint32& SizeY) const
{
	LOG_FUNC();
	//LOGI(WVRHMD, "AdjustViewRect eye %d (%d, %d, %d, %d) before", ViewIndex, X, Y, SizeX, SizeY);
#if WITH_EDITOR
	if (GIsEditor) {
	...
	//LOGI(WVRHMD, "AdjustViewRect eye %d (%d, %d, %d, %d) after", ViewIndex, X, Y, SizeX, SizeY);
}

// Need ScreenPercentage is true
void FWaveVRHMD::SetFinalViewRect(class FRHICommandListImmediate& RHICmdList, const int32 ViewIndex, const FIntRect& FinalViewRect) {
	LOG_FUNC();
	//LOGI(WVRHMD, "SetFinalViewRect ViewIndex(%d) FinalViewRect MinX(%d),MaxX(%d), MinY(%d),MaxY(%d)", ViewIndex, FinalViewRect.Min.X, FinalViewRect.Max.X, FinalViewRect.Min.Y, FinalViewRect.Max.Y);
}

// Will call to GetRelativeEyePose
void FWaveVRHMD::CalculateStereoViewOffset(const int32 ViewIndex, FRotator& ViewRotation, const float WorldToMeters, FVector& ViewLocation)
{
	LOG_FUNC();
	FHeadMountedDisplayBase::CalculateStereoViewOffset(ViewIndex, ViewRotation, WorldToMeters, ViewLocation);
}

FMatrix FWaveVRHMD::GetStereoProjectionMatrix(const int32 ViewIndex) const
{
	LOG_FUNC();
	...
	} else if (ViewIndex == EStereoscopicEye::eSSE_MONOSCOPIC) {
...
void FWaveVRHMD::SetFoveationParams(const int32 ViewIndex, WVR_RenderFoveationParams_t& FoveatParams) {
	mRender.SetFoveationParams(ViewIndex, FoveatParams);
}

void FWaveVRHMD::GetFoveationParams(const int32 ViewIndex, WVR_RenderFoveationParams_t& FoveatParams) {
	mRender.GetFoveationParams(ViewIndex, FoveatParams);
}
