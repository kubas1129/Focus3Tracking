...
namespace wvr {
	namespace utils {
		...
		// If the input data16 array is column major, set columnMajor true.
		template<typename T>
		static FORCEINLINE FMatrix FromGLToUnrealMatrix(const T * data16, bool columnMajor, float meterToWorldUnit)
		{
			// The FMatrix's column is mathematical matrix's row, and the FMatrix's row is mathematical matrix's column.
			if (columnMajor)
				return FMatrix(
					FPlane( data16[10], -data16[ 8], -data16[ 9], -data16[11]),
					FPlane(-data16[ 2],  data16[ 0],  data16[ 1],  data16[ 3]),
					FPlane(-data16[ 6],  data16[ 4],  data16[ 5],  data16[ 7]),
					FPlane(-data16[14] * meterToWorldUnit,  data16[12] * meterToWorldUnit,  data16[13] * meterToWorldUnit,  data16[15]));
			else
				return FMatrix(
					FPlane( data16[10], -data16[2], -data16[ 6], -data16[14]),
					FPlane(-data16[ 8],  data16[0],  data16[ 4],  data16[12]),
					FPlane(-data16[ 9],  data16[1],  data16[ 5],  data16[13]),
					FPlane(-data16[11] * meterToWorldUnit,  data16[3] * meterToWorldUnit,  data16[ 7] * meterToWorldUnit,  data16[15]));
		}
