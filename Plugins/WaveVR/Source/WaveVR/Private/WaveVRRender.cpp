...

{
	...
	if (PLATFORM_WINDOWS) {
		...
		msaaLevel = GetDefaultMSAACount(ERHIFeatureLevel::ES3_1); //This function will check that the EAntiAliasingMethod must be AAM_MSAA
		auto CVarMobileMultiView = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("vr.MobileMultiView"));
		auto CVarMobileHDR = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.MobileHDR"));
		bool bMobileHDR = (CVarMobileHDR && CVarMobileHDR->GetValueOnAnyThread() != 0);
		isMultiViewEnabled = (!bMobileHDR && (CVarMobileMultiView && CVarMobileMultiView->GetValueOnAnyThread() != 0)) && RHISupportsMobileMultiView(GMaxRHIShaderPlatform);
		LOGD(WVRRender, "isSRGB %d, isMobileMultiViewEnabled %d, isMobileHDR %d, MobileMSAALevel %d", isSRGB, isMultiViewEnabled, bMobileHDR, msaaLevel);
...
//#endif
	...
	{
		uint64_t render_config = WVR_RenderConfig_Default | isSRGB ? WVR_RenderConfig_sRGB : 0 | WVR_RenderConfig_InverseAlpha;
	...
	if (mCurrentFoveationMode == WVR_FoveationMode::WVR_FoveationMode_Enable) {
		SetFoveationParams(EStereoscopicEye::eSSE_LEFT_EYE, EnableModeFoveationParams[EStereoscopicEye::eSSE_LEFT_EYE]);
		SetFoveationParams(EStereoscopicEye::eSSE_RIGHT_EYE, EnableModeFoveationParams[EStereoscopicEye::eSSE_RIGHT_EYE]);
	}
	...
	// common
	info.flags = ETextureCreateFlags::RenderTargetable | ETextureCreateFlags::ShaderResource | (isSRGB ? ETextureCreateFlags::SRGB : ETextureCreateFlags::None );
	...
	FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
{
	...
	//LOG_FUNC();
	LOGI(WVRRender, "AllocateRenderTargetTexture(index=%d width=%u height=%u mips=%u samples=%u flags=%llu targetFlags=%llu, format=%u)",
...
bool FWaveVRRender::AllocateDepthTexture(uint32 Index, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, ETextureCreateFlags Flags, ETextureCreateFlags TargetableTextureFlags, FTexture2DRHIRef& OutTargetableTexture, FTexture2DRHIRef& OutShaderResourceTexture, uint32 NumSamples)
{
	...
	//LOG_FUNC();
	LOGI(WVRRender, "AllocateDepthTexture(index=%d PD=%f width=%u height=%u mips=%u samples=%u flags=%llu targetFlags=%llu, format=%u)",
...
void FWaveVRRender::SetFoveationParams(const int32 ViewIndex, const WVR_RenderFoveationParams_t& foveationParams)
{
	LOG_FUNC();
	...
	EnableModeFoveationParams[ViewIndex] = foveationParams;
	if (ViewIndex == EStereoscopicEye::eSSE_RIGHT_EYE) {
	...
	LOGI(WVRRender, "Set ViewIndex(%d) focalX (%f) focalY (%f) FOV(%f) periQuality(%d)",
		ViewIndex , foveationParams.focalX, foveationParams.focalY,
...
void FWaveVRRender::GetFoveationParams(const int32 ViewIndex, WVR_RenderFoveationParams_t& foveationParams) const {
	LOG_FUNC();

	if(mCurrentFoveationMode == WVR_FoveationMode::WVR_FoveationMode_Default) {
		foveationParams = DefaultModeFoveationParams[ViewIndex];
		LOGI(WVRRender, "Get DefaultModeConfig");
	} else {
		foveationParams = EnableModeFoveationParams[ViewIndex];
		LOGI(WVRRender, "Get EnableModeConfig");
	}

	LOGI(WVRRender, "Get ViewIndex(%d) focalX (%f) focalY (%f) FOV(%f) periQuality(%d)",
		ViewIndex, foveationParams.focalX, foveationParams.focalY,
		foveationParams.fovealFov, foveationParams.periQuality);
}
...
void FWaveVRRender::OnBeginRendering_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& ViewFamily)
{
	...
	const FSceneView& sceneViewLeft = *ViewFamily.Views[EStereoscopicEye::eSSE_LEFT_EYE];
	const FSceneView& sceneViewRight = *ViewFamily.Views[EStereoscopicEye::eSSE_RIGHT_EYE];
