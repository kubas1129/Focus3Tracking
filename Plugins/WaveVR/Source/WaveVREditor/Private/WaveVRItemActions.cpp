...
void UWaveVRItemActions::StartDPServer() {
	...
	UE_LOG(WVRMenuItemAction, Display, TEXT("Drive is %s"), *driveStr);
	FString execCmd;
	if (driveStr.Equals(".."))
	{
		execCmd = TEXT("/c cd ") + workingDir + TEXT("/Prebuilt/DirectPreview/dpServer") + TEXT(" && dpServer");
	}
	else
	{
		execCmd = TEXT("/c ") + driveStr + TEXT(" && cd ") + workingDir + TEXT("/Prebuilt/DirectPreview/dpServer") + TEXT(" && dpServer");
	}
...
void UWaveVRItemActions::InstallDeviceAPK() {
	...
	FString uninstallCmd = TEXT("/c adb uninstall com.htc.vr.directpreview.agent.unreal");
	UE_LOG(WVRMenuItemAction, Display, TEXT("uninstall apk: %s"), *uninstallCmd);
	FProcHandle punInstall = FPlatformProcess::CreateProc(TEXT("C:\\WINDOWS\\system32\\cmd.exe"), *uninstallCmd, true, false, false, nullptr, 0, nullptr, nullptr);
	...
	if (driveStr.Equals(".."))
	{
		execCmd = TEXT("/c cd ") + configDir + TEXT(" && adb push DirectPreviewConfig.json /sdcard/DirectPreview/config.json");
	}
	else {
		execCmd = TEXT("/c ") + driveStr + TEXT(" && cd ") + configDir + TEXT(" && adb push DirectPreviewConfig.json /sdcard/DirectPreview/config.json");
	}
	...
	if (driveStr.Equals(".."))
	{
		installCmd = TEXT("/c cd ") + configDir + TEXT(" && adb install -r -g wvr_plugins_directpreview_agent_unreal.apk");
	}
	else {
		installCmd = TEXT("/c ") + driveStr + TEXT(" && cd ") + configDir + TEXT(" && adb install -r -g wvr_plugins_directpreview_agent_unreal.apk");
	}
