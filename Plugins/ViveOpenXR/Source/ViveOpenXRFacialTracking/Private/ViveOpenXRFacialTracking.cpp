...
bool FViveOpenXRFacialTracking::GetRequiredExtensions(TArray<const ANSICHAR*>& OutExtensions)
{
	...
	{
		UE_LOG(LogViveOpenXRFacialTracking, Log, TEXT("Add FacialTracking Extension Name."));
...
bool FViveOpenXRFacialTracking::DestroyFacialTracker()
{
	...
	if (m_FacialTracker_Eye == XR_NULL_HANDLE && m_FacialTracker_Lip == XR_NULL_HANDLE)
	{
		UE_LOG(LogViveOpenXRFacialTracking, Error, TEXT("[DestroyFacialTracker] FacialTracker is XR_NULL_HANDLE."));
		return true;
	}
	XrResult result = XrResult::XR_ERROR_HANDLE_INVALID;
	if (m_FacialTracker_Eye != XR_NULL_HANDLE)
...
bool FViveOpenXRFacialTracking::GetEyeExpressions(bool& isActive)
{
	...
	facialExpressions.expressionCount = 60;
